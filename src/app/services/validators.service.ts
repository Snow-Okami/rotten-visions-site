...
export class ValidatorsService extends Validators  {
  ...
  constructor() {
    ...
    that = this;
    this.badWords = this.badWordsString.split(', ');
  }
  ...
  title(control: FormControl) {
    if (control.value && control.value.length > 0) {
      ...
      return !matches ? { invalid_characters: true } : _.find(that.badWords, it => _.includes(v, it)) ? { badword: true }: null;
    } else {
      return null;
    }
  }
  ...
  description(control: FormControl) {
    if (control.value && control.value.length > 0) {
      ...
      return wordList.length ? matches ? _.find(that.badWords, it => _.includes(v, it)) ? { badword: true }: null : { invalid_description: true } : { invalid_description: true };
    } else {
      return null;
    }
  }
